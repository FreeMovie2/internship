<?php
    ...
    if(isset($_POST['submit']) && $_SERVER['REQUEST_METHOD'] == 'POST'){
        $uploadDir = "../information_file/company/"; 
        ...
        if(!in_array($ext, $allowed) && !in_array($ext2, $allowed)){
            ...
            if($sql->execute()) {
                header("location:../edit_company.php?status=success");
            } else {
                header("location:../edit_company.php?status=error");;
            }
        ...
        echo "<script>alert('การร้องขอไม่ถูกต้อง'); window.history.back();</script>";
    }
?>
