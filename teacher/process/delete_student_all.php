<?php
    ...
    if(isset($_POST['submit']) && $_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if($fetch_teacher['t_password'] == $t_password){
            $sql = $conn->prepare("DELETE FROM students WHERE s_id != 2");
            if($sql->execute()){
                $sql_2 = $conn->prepare("DELETE FROM student_information WHERE s_id != 2");
                if($sql_2->execute()){
                    $sql_3 = $conn->prepare("DELETE FROM parent_information WHERE s_id != 2");
                    if($sql_3->execute()){
                        $sql_4 = $conn->prepare("DELETE FROM company WHERE s_id != 2");
                        if($sql_4->execute()){
